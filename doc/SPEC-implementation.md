...
- instance-wide host-process cap: at most `PAPERCLIP_MAX_CONCURRENT_HOST_RUNS` concurrent heartbeat runs whose adapters spawn on the Paperclip host (default `12`; `0` disables). Remote adapters (`http`, `cursor_cloud`, `hermes_gateway`, `openclaw_gateway`) are excluded. Extra work stays `queued` and is promoted when a host slot frees.
