...
describe("codex local adapter metadata", () => {
  it("advertises current Codex-capable OpenAI models without changing the default", () => {
    ...
    // Default to the concrete gpt-6-sol slug — Codex ships no metadata for the bare gpt-5.6
    // alias, so it must not be advertised or used as the default (it triggers a fallback warning).
    expect(DEFAULT_CODEX_LOCAL_MODEL).toBe("gpt-6-sol");
  ...
  it.each(["gpt-6-astra", "gpt-6-sol", " gpt-6-sol ", "gpt-5.6-terra", "gpt-5.6"])("uses the reasoning efforts advertised for %s", (model) => {
  ...
  it.each(["gpt-6-luna"])("caps %s at max and supports Fast mode", (model) => {
  ...
  it("normalizes the legacy bare gpt-5.6 alias to the concrete gpt-6-sol slug", () => {
    expect(normalizeCodexModel("gpt-5.6")).toBe("gpt-6-sol");
    expect(normalizeCodexModel("  gpt-5.6  ")).toBe("gpt-6-sol");
