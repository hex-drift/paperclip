...
RUN echo "cli-tools-epoch: ${CLI_TOOLS_CACHE_EPOCH}" \
  && npm install --global --omit=dev @anthropic-ai/claude-code@latest @openai/codex@latest opencode-ai @google/gemini-cli@latest @moonshot-ai/kimi-code@latest @customerio/cli@latest \
  && apt-get update \
...
RUN chmod +x /usr/local/bin/docker-entrypoint.sh /usr/local/bin/docker-mcp-bootstrap.sh /usr/local/bin/docker-customerio-bootstrap.sh
